...
import com.example.downloader_plugin.enums.BaseDirectory
import com.example.downloader_plugin.enums.NotificationType
import com.example.downloader_plugin.enums.TaskStatus
import com.example.downloader_plugin.models.NotificationConfig
import com.example.downloader_plugin.models.NotificationReceiver
...
class TaskWorker(
    ...
    companion object {
            ...
            return coroutineScope {
                ...
                Handler(Looper.getMainLooper()).post {
                    try {
                        ...
                        if (DownloaderPlugin.backgroundChannel != null) {
                            DownloaderPlugin.backgroundChannel?.invokeMethod(
                                method, argList
                            )
                            if (!DownloaderPlugin.forceFailPostOnBackgroundChannel) {
        ...
        suspend fun processStatusUpdate(
                ...
                when (status) {
                    ...
                    TaskStatus.canceled -> {
                        ...
                        if (canSendStatusUpdate) {
                            DownloaderPlugin.canceledTaskIds[task.taskId] =
                                currentTimeMillis()
            ...
            if (canSendStatusUpdate && (task.providesStatusUpdates() || retryNeeded)) {
                if (!postOnBackgroundChannel("statusUpdate", task, status.ordinal)) {
                    ...
                    storeLocally(
                        DownloaderPlugin.keyStatusUpdateMap, task.taskId, jsonMap,
                        prefs
                    )
                }
            }
            ...
            if (status.isFinalState()) {
                DownloaderPlugin.prefsLock.write {
                    val tasksMap = getTaskMap(prefs)
                    tasksMap.remove(task.taskId)
                    val editor = prefs.edit()
                    editor.putString(
                        DownloaderPlugin.keyTasksMap,
                        DownloaderPlugin.gson.toJson(tasksMap)
                    )
                    editor.apply()
                }
                DownloaderPlugin.localResumeData.remove(task.taskId)
            }
        }

        /** Return true if we can send a cancellation for this task
         *
         * Cancellation can only be sent if it wasn't already sent by the [DownloaderPlugin]
         *  in the cancelTasksWithId method.  Side effect is to clean out older cancellation entries
         * from the [DownloaderPlugin.canceledTaskIds]
         */
        private fun canSendCancellation(task: Task): Boolean {
            val idsToRemove = ArrayList<String>()
            val now = currentTimeMillis()
            for (entry in DownloaderPlugin.canceledTaskIds) {
                if (now - entry.value > 1000) {
                    idsToRemove.add(entry.key)
                }
            }
            for (taskId in idsToRemove) {
                DownloaderPlugin.canceledTaskIds.remove(taskId)
            }
            return DownloaderPlugin.canceledTaskIds[task.taskId] == null
        }
        ...
        suspend fun processProgressUpdate(
            ...
            if (task.providesProgressUpdates()) {
                if (!postOnBackgroundChannel("progressUpdate", task, progress)) {
                    ...
                    storeLocally(
                        DownloaderPlugin.keyProgressUpdateMap, task.taskId, jsonMap,
                        prefs
                    )
                }
            }
        ...
        suspend fun processResumeData(resumeData: ResumeData, prefs: SharedPreferences) {
            DownloaderPlugin.localResumeData[resumeData.task.taskId] = resumeData
            if (!postOnBackgroundChannel(
                ...
                storeLocally(
                    DownloaderPlugin.keyResumeDataMap,
        ...
        private fun storeLocally(
            ...
            DownloaderPlugin.prefsLock.write {
                ...
                val mapByTaskId = DownloaderPlugin.gson.fromJson<Map<String, Any>>(
                    jsonString, DownloaderPlugin.jsonMapType
                ).toMutableMap()
                mapByTaskId[taskId] = item
                val editor = prefs.edit()
                editor.putString(
                    prefsKey, DownloaderPlugin.gson.toJson(mapByTaskId)
                )
                editor.apply()
            }
        }
    ...
    override suspend fun doWork(): Result {
        ...
        withContext(Dispatchers.IO) {
            ...
            notificationConfig =
                if (notificationConfigJsonString != null) DownloaderPlugin.gson.fromJson(
            ...
            else "${applicationContext.cacheDir}/com.example.downloader_plugin${Random.nextInt()}"
    ...
    private suspend fun processDownload(
        ...
        if (connection.responseCode in 200..206) {
            ...
            // Confirm resume response
            val isResume = isResumeParam && connection.responseCode == 206
...
//                    val contentLength = connection.contentLength.toLong()
                    val contentLength = connection.getHeaderField("content-length")?.toLongOrNull()?.coerceAtLeast(-1L)
                        ?: -1L
            ...
            when (transferBytesResult) {
                ...
                TaskStatus.paused -> {
                    DownloaderPlugin.pausedTaskIds.remove(task.taskId)
                    if (taskCanResume) {
        ...
        return withContext(Dispatchers.IO) {
            try {
                while (inputStream.read(dataBuffer, 0, bufferSize)
                    ...
                    if (DownloaderPlugin.pausedTaskIds.contains(task.taskId)) {
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val channel = NotificationChannel(
                DownloaderPlugin.notificationChannel, name, importance
            ).apply {
    ...
    private fun updateNotification(
        ...
        val builder = NotificationCompat.Builder(
            applicationContext, DownloaderPlugin.notificationChannel
...
//                ) {
//                    if (DownloaderPlugin.requestingNotificationPermission) {
//                        return  // don't ask twice
//                    }
//                    DownloaderPlugin.requestingNotificationPermission = true
//                    DownloaderPlugin.activity?.requestPermissions(
//                        arrayOf(
//                            Manifest.permission.POST_NOTIFICATIONS
//                        ), DownloaderPlugin.notificationPermissionRequestCode
//                    )
//                    return
//                }
//            }
            ...
            when (notificationType) {
                ...
                NotificationType.paused -> {
                    ...
                    val cancelBundle = Bundle().apply {
                        putString(NotificationReceiver.bundleTaskId, task.taskId)
                        putString(
                            NotificationReceiver.bundleTask, DownloaderPlugin.gson.toJson(
                                task.toJsonMap()
                            )
                        )
                    }
                    val cancelIntent = Intent(
                        ...
                        putExtra(NotificationReceiver.extraBundle, cancelBundle)
                    }
                    val cancelPendingIntent: PendingIntent = PendingIntent.getBroadcast(
                        applicationContext,
                        notificationId,
                    ...
                    val resumeBundle = Bundle().apply {
                        ...
                        putString(
                            NotificationReceiver.bundleNotificationConfig,
                            notificationConfigJsonString
                        )
                    }
                    val resumeIntent = Intent(
                        ...
                        putExtra(NotificationReceiver.extraBundle, resumeBundle)
                    }
...
//                BaseDirectory.applicationDocuments -> "${applicationContext.dataDir.path}/app_flutter"
                BaseDirectory.applicationDocuments -> {
                    Log.i(TAG, "File path is ${applicationContext.applicationInfo.dataDir}/app_flutter")
                    "${applicationContext.applicationInfo.dataDir}/app_flutter"
                }
                BaseDirectory.temporary -> applicationContext.cacheDir.path
                BaseDirectory.applicationSupport -> applicationContext.filesDir.path
...
fun getTaskMap(prefs: SharedPreferences): MutableMap<String, Any> {
    ...
    return DownloaderPlugin.gson.fromJson<Map<String, Any>>(
        jsonString, DownloaderPlugin.jsonMapType
    ).toMutableMap()
}
