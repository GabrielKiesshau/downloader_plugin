...
class NativeDownloader extends BaseDownloader {
  ...
  static const _channel = MethodChannel('downloader_plugin');
  static const _backgroundChannel = MethodChannel('downloader_plugin.background');

  factory NativeDownloader() => _singleton;
  ...
  Future<void> initialize() async {
    ...
    _backgroundChannel.setMethodCallHandler((call) async {
      ...
      switch (call.method) {
        ...
        default:
          throw UnimplementedError('Background channel method call ${call.method} not supported');
      }
    });
  }
  ...
  //   final result = await _channel.invokeMethod<List<dynamic>?>('allTasks', group) ?? [];
  //   final tasks = result.map((e) => Task.createFromJsonMap(jsonDecode(e as String))).toList();
  //   return [...retryAndPausedTasks, ...tasks];
  // }

  // @override
  // Future<bool> cancelPlatformTasksWithIds(List<String> taskIds) async => await _channel.invokeMethod<bool>('cancelTasksWithIds', taskIds) ?? false;
  ...
  // @override
  // Future<bool> pause(Task task) async => await _channel.invokeMethod<bool>('pause', task.taskId) ?? false;
  ...
  //       return await _channel.invokeMethod<bool>('enqueue', [jsonEncode(task.toJsonMap()), jsonEncode(notificationConfig?.toJsonMap()), taskResumeData.data, taskResumeData.requiredStartByte]) ??
  //           false;
  //     }
  //   }
  //   return false;
  ...
  //     final timeoutMillis = await _channel.invokeMethod<int>('getTaskTimeout') ?? 0;
